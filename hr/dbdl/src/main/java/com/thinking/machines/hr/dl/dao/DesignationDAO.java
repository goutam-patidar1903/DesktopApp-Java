...
title=title.trim();
if(title.length()==0)throw new DAOException("Length is Designation is zero");
try
{
...
preparedStatement.close();
connection.close();
throw new DAOException("Cannot delete designation : "+title+" as it is alloted to employee(s)");
}
resultSet.close();
preparedStatement.close();
...
{
if(title==null||title.trim().length()==0) throw new DAOException("Invalid tile : "+title);
title=title.trim();
try
{
Connection connection=DAOConnection.getConnection();
